...
```{r}
# Basic summary for all numeric variables (excluding site ID)
summary_stats <- sites.full %>%
  dplyr::select(-site_no) %>%
  summary()

# Grouped summary statistics by income group
group_summary <- sites.full.t %>%
  group_by(income_group) %>%
  summarise(across(
    c(mean_temp_C, mean_t_air, log_flow, log_elev, per_pov, per_industry),
...
kable(group_summary, caption = "Table 2. Summary Statistics by Income Group") %>%
...
```{r}
sites.full.t %>%
  ggplot(aes(x = mean_temp_C, fill = income_group)) +
  geom_density(alpha = 0.5) +
  theme_minimal() +
  labs(title = "Water Temperature Density by Income Group", x = "Mean Water Temperature (°C)", y = "Density") +
  scale_fill_manual(values = c("Low" = "navy", "High" = "orange"))
